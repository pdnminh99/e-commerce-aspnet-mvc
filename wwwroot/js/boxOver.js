...
function scanBO(curNode) {
  if (checkElemBO(curNode.title)) {
    ...
    curNode.IEbugfix =
      getParam("hideselects", curNode.title) == "on" ? true : false;
    curNode.fixX = parseInt(getParam("fixedrelx", curNode.title));
    curNode.fixY = parseInt(getParam("fixedrely", curNode.title));
    curNode.absX = parseInt(getParam("fixedabsx", curNode.title));
    curNode.absY = parseInt(getParam("fixedabsy", curNode.title));
    curNode.offY =
      getParam("offsety", curNode.title) != ""
        ? parseInt(getParam("offsety", curNode.title))
        : 10;
    curNode.offX =
      getParam("offsetx", curNode.title) != ""
        ? parseInt(getParam("offsetx", curNode.title))
        : 10;
    curNode.fade = getParam("fade", curNode.title) == "on" ? true : false;
    curNode.fadespeed =
      getParam("fadespeed", curNode.title) != ""
        ? getParam("fadespeed", curNode.title)
        : 0.04;
    curNode.delay =
      getParam("delay", curNode.title) != ""
        ? parseInt(getParam("delay", curNode.title))
        : 0;
    if (getParam("requireclick", curNode.title) == "on") {
      ...
      if (getParam("singleclickstop", curNode.title) == "on") {
        document.all
          ? curNode.attachEvent("onclick", pauseBox)
          : curNode.addEventListener("click", pauseBox, false);
      }
    }
    curNode.windowLock =
      getParam("windowlock", curNode.title).toLowerCase() == "off"
        ? false
        : true;
    curNode.title = "";
    curNode.hasbox = 1;
...
function fadeIn2(fs) {
  ...
  if (COL < 1) setTimeout("fadeIn2(" + fs + ")", 10);
}
